...
void print_placement(std::vector<int> &curr_word, int col, int row, int direction);std::vector <std::vector<int>> get_slots(std::vector <std::vector<int>> &grid, int grid_size, std::vector<int> &curr_word);
...
int main()
{
    ...
    if ( words.read_raw_list("C:\\Users\\peter\\c++\\WordSearch\\words_santa.txt") ){
        words.create_clean_list();
